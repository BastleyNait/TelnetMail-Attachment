...
class TelnetMail:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.attachments = kwargs.get("attachments", [])  # lista de paths
    ...
    def add_headers(self):
        """ This method add headers and sets up MIME multipart for attachments. """
        headers_string = (
            "MIME-Version: 1.0\n"
        ...
        if hasattr(self, "attachments"):
            for path in self.attachments:
                attachments += build_attachment(path)

        # Cierre del mensaje
        closing = "--BOUNDARY12345--\r\n"

        self.message = headers_string + "\n" + body + attachments + closing
    ...
    def build_attachment(filepath):
        ...
        attachment = (
            f"--BOUNDARY12345\r\n"
            f"Content-Type: {content_type}; name=\"{path.name}\"\r\n"
            f"Content-Disposition: attachment; filename=\"{path.name}\"\r\n"
            f"Content-Transfer-Encoding: base64\r\n\r\n"
            f"{b64encode(file_content).decode()}\r\n"
        )
        return attachment
    def send_mail(self):
